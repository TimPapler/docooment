...
Collection.prototype.ensureIndex = function(arg1, opts, callback){
    var keys = Object.keys(arg1);
    this.indexPaths = this.indexPaths || [{
        Path:"\/",
        indexType:"Hash",
        NumericPrecision:3,
        StringPrecision:3
    ...
    for (var i = 0; i<keys.length; ++i) {
        this.indexPaths.push(utils.options({Path: "\/\""+keys[i]+"\"\/?",}, opts));
    }
    callback();
};
...
Collection.prototype.find = function(select, options, callback){
  ...
  var query = queryComposer.composeQuery(select, fields, model);
...
Collection.prototype.insert = function(docs, options, callback){
  if(!this.conn || !this.conn.database._self){
    this.addQueue('insert', [docs, options, callback]);
    return;
  }
...
var createCollection = function(self, name, options, callback){
  var body = {id: name};
  body.indexingPolicy = { IncludedPaths:self.indexPaths };
  console.log('creating collection:', name);
  self.conn.server.createCollection(self.conn.database._self, body, options, function(err, collection) {
    if(err) return callback(err);
