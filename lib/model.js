...
Model.prototype.save = function save (fn) {
  ...
  if (this.isNew) {
    ...
    /*
    if (!utils.object.hasOwnProperty(obj || {}, 'id')) {
      // documents must have an id else docooment won't know
      // what to update later if more changes are made. the user
      // wouldn't know what id was generated by mongodb either
      // 
      // match the schema definition.
      return complete(new Error('document must have an id before saving'));
    }
  */
...
function checkDivergentArray (doc, path, array) {
  ...
  // deselected the id field, $pop and $set of the array are
  // not safe operations. If id was deselected, we do not know
  // how to remove elements. $pop will pop off the id from the end
  ...
  var check = pop.options.match ||
              ...
              pop.options.select && // deselected id?
                (0 === pop.options.select.id ||
                /\s?-id\s?/.test(pop.options.select))
 ...
 *
 *     var query = Comment.remove({ id: id });
 *     query.exec();
 *
 * ####Note:
 *
 ...
 *
 * @param {string|HexId} id objectid, or a value that can be casted to one
 * @param {Object} [fields] optional fields to select
 * @param {Object} [options] optional
 * @param {Function} [callback]
 * @return {Query}
...
Model.findById = function findById (id, fields, options, callback) {
  return this.findOne({ id: id }, fields, options, callback);
};
 ...
 *     })
 *
 * @param {string|HexId} id a string that can be cast to one.
 * @param {Object} [update]
 * @param {Object} [options]
 * @param {Function} [callback]
 * @return {Query}
...
Model.findByIdAndUpdate = function (id, update, options, callback) {
  ...
  if (1 === arguments.length) {
    ...
    return this.findOneAndUpdate({id: id }, undefined);
  }
 ...
 *
 * @param {HexString} id string that can be cast to one
 * @param {Object} [options]
 * @param {Function} [callback]
 * @return {Query}
...
Model.findByIdAndRemove = function (id, options, callback) {
  ...
  return this.findOneAndRemove({ id: id }, options, callback);
}
 ...
 * ####Note:
 *
 * Be careful to not use an existing model instance for the update clause (this won't work and can cause weird behavior like infinite loops). Also, ensure that the update clause does not have an id property, which causes Mongo to return a "Mod on id not allowed" error.
 *
 * ####Note:
 *
 ...
 *
 *     Comment.update({ id: id }, { $set: { text: 'changed' }}).exec();
 *
 * ####Note:
 *
 ...
 *     Users.aggregate(
 *         { $group: { id: null, maxBalance: { $max: '$balance' }}}
 *       , { $project: { id: 0, maxBalance: 1 }}
 *       , function (err, res) {
 *       if (err) return handleError(err);
...
function populate (model, docs, options, cb) {
  ...
  for (i = 0; i < numDocuments; ++i) {
    ret = undefined;
    doc = docs[i];
    id = String(utils.getValue("id", doc));
    isDocument = !! doc.$__;
    ...
    if (ret) {
      ...
      // previously we always assigned this even if the document had no id
    ...
    if (isDocument) {
      // cache original populated ids and model used
  ...
  match.id || (match.id = { $in: ids });

  var assignmentOpts = {};
  assignmentOpts.sort = options.options && options.options.sort || undefined;
  assignmentOpts.excludeId = /\s?-id\s?/.test(select) || (select && 0 === select.id);

  if (assignmentOpts.excludeId) {
    // override the exclusion from the query so we can use the id
    // for document matching during assignment. we'll delete the
    // id back off before returning the result.
    if ('string' == typeof select) {
      select = select.replace(/\s?-id\s?/g, ' ');
    } else {
  ...
  Model.find(match, select, options.options, function (err, vals) {
    ...
    for (var i = 0; i < len; i++) {
      val = vals[i];
      key = String(utils.getValue('id', val));
      rawDocs[key] = val;
      rawOrder[key] = i;
...
/*!
 * Retrieve the id of `val` if a Document or Array of Documents.
 *
...
function convertToid (val) {
  if (val instanceof Model) return val.id;
...
function assignVals (o) {
  // replace the original ids in our intermediate ids structure
 ...
 *      b) remove id from sub docs when user specified
 *
 *    findOne
 *      a) if no doc found, set to null
 *      b) remove id from sub docs when user specified
 *
 * 2) Remove ids when specified by users query.
 *
 * background:
 * ids are left in the query even when user excludes them so
...
/*!
 * Remove id from `subdoc` if user specified "lean" query option
 */
